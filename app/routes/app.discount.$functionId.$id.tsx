...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  try {
    ...
    // Update discount settings (assuming first discount for now)
    const widget = await getWidgetById(session.shop, widgetId);
...
export default function EditDiscount() {
  const { functionId, widget } = useLoaderData<typeof loader>();
  ...
  const [visibility, setVisibility] = useState<string>(existingDiscount?.appliesTo || 'ALL_PRODUCTS');
  ...
  // Initialize resource selection state from existing data
  const [selectedProducts, setSelectedProducts] = useState<{ id: string; title: string }[]>(() => {
    // Convert existing productIds to the format expected by ProductPicker
    if (existingDiscount?.productIds && existingDiscount.productIds.length > 0) {
      return existingDiscount.productIds.map(id => ({ id, title: `Product ${id}` }));
    }
    return [];
  });
  const [selectedCollections, setSelectedCollections] = useState<{ id: string; title: string }[]>(() => {
    // Convert existing collectionIds to the format expected by CollectionPicker
    if (existingDiscount?.collectionIds && existingDiscount.collectionIds.length > 0) {
      return existingDiscount.collectionIds.map(id => ({ id, title: `Collection ${id}` }));
    }
    return [];
  });

  // Initialize style settings from existing data
  const [titleFontSize, setTitleFontSize] = useState<string>(existingSettings?.typography?.titleFontSize || '16');
  const [titleColor, setTitleColor] = useState<string>(existingSettings?.colors?.blockTitleColor || '#111213');
  const [titleWeight, setTitleWeight] = useState<string>(existingSettings?.typography?.titleWeight || '600');
  const [optionHeadingSize, setOptionHeadingSize] = useState<string>(existingSettings?.typography?.optionHeadingSize || '14');
  const [optionDescSize, setOptionDescSize] = useState<string>(existingSettings?.typography?.optionDescSize || '12');
  const [optionHeadingWeight, setOptionHeadingWeight] = useState<string>(existingSettings?.typography?.optionHeadingWeight || '700');
  const [optionDescWeight, setOptionDescWeight] = useState<string>(existingSettings?.typography?.optionDescWeight || '400');
  const [labelSize, setLabelSize] = useState<string>(existingSettings?.typography?.labelSize || '12');
  const [labelWeight, setLabelWeight] = useState<string>(existingSettings?.typography?.labelWeight || '400');

  const [cardsBg, setCardsBg] = useState<string>(existingSettings?.colors?.cardsBg || '#FFFFFF');
  const [selectedBg, setSelectedBg] = useState<string>(existingSettings?.colors?.selectedBg || '#EEF3FF');
  const [borderColor, setBorderColor] = useState<string>(existingSettings?.colors?.borderColor || '#E1E3E5');
  const [blockTitleColor, setBlockTitleColor] = useState<string>(existingSettings?.colors?.blockTitleColor || '#111213');
  const [barTitleColor, setBarTitleColor] = useState<string>(existingSettings?.colors?.barTitleColor || '#111213');
  const [barSubtitleColor, setBarSubtitleColor] = useState<string>(existingSettings?.colors?.barSubtitleColor || '#5C5F62');
  const [priceColor, setPriceColor] = useState<string>(existingSettings?.colors?.priceColor || '#111213');
  const [fullPriceColor, setFullPriceColor] = useState<string>(existingSettings?.colors?.fullPriceColor || '#5C5F62');
  const [labelBg, setLabelBg] = useState<string>(existingSettings?.colors?.labelBg || '#D9D9D9');
  const [labelText, setLabelText] = useState<string>(existingSettings?.colors?.labelText || '#111213');

  // Initialize gift settings from existing data
  const [giftTextSize, setGiftTextSize] = useState<string>(existingSettings?.gift?.textSize || '12');
  const [giftTextWeight, setGiftTextWeight] = useState<string>(existingSettings?.gift?.textWeight || '400');
  const [giftTextColor, setGiftTextColor] = useState<string>(existingSettings?.gift?.textColor || '#111213');
  const [giftBg, setGiftBg] = useState<string>(existingSettings?.gift?.bg || '#F8F9FA');
  const [giftBorderColor, setGiftBorderColor] = useState<string>(existingSettings?.gift?.borderColor || '#E1E3E5');
  ...
  const discountFormJson = useMemo(() => {
    ...
    if (visibility === 'SPECIFIC_PRODUCTS' || visibility === 'ALL_PRODUCTS_NOT_SOME') {
      ...
      // All products: keep existing productIds if any, clear collections
      ...
      // For other cases, preserve existing values but clear conflicting ones
      productIds = existingDiscount?.productIds || [];
      collectionIds = existingDiscount?.collectionIds || [];
    }
    ...
    <Page>
      ...
      <div style={{
        ...
        <InlineStack gap="300" blockAlign="center">
          ...
          />
          <Text as="h1" variant="headingLg">Edit Discount Bundle</Text>
        </InlineStack>
